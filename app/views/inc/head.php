<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title><?php echo APP_NAME ?></title>
<link rel="stylesheet" href="<?php echo APP_URL; ?>app/views/css/bulma.min.css">
<link rel="stylesheet" href="<?php echo APP_URL; ?>app/views/css/style.css">
<link rel="stylesheet" href="<?php echo APP_URL; ?>app/views/css/sweetalert2.min.css">
<script src="<?php echo APP_URL; ?>app/views/js/sweetalert2.all.min.js"></script>
     
<link rel="stylesheet" href="<?php echo APP_URL; ?>app/views/css/bulma-carousel.min.css">
<script src="<?php echo APP_URL; ?>app/views/js/bulma-carousel.js"></script>
<script src="<?php echo APP_URL; ?>app/views/js/bulma-carousel.min.js"></script>

